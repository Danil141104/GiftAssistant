...
struct EmailVerificationView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text(authService.currentUser?.email ?? "")
                .font(.headline).foregroundColor(Color.theme.text)
            
            Text("Open the email and click the link,\nthen tap the button below")
                .font(.subheadline)
                .foregroundColor(Color.theme.textSecondary)
            ...
            Button {
                ...
                HStack {
                    ...
                    Text(showResent ? "Email sent!" : "Resend verification email")
                }
                .font(.subheadline)
                .foregroundColor(showResent ? Color.theme.success : Color.theme.secondary)
            }
            .disabled(showResent)
            ...
            Button {
                ...
                Text("Sign out and use a different email")
    ...
    private func checkEmail() {
        isChecking = true
        Task {
            await authService.checkVerification()
            isChecking = false
        }
    }
